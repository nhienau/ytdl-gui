...
class PresetDetailFrame(ctk.CTkScrollableFrame):
    ...
    def _on_new_preset_submit(self):
        ...
        if self._var_preset_name.get().strip() == "":
            ...
            valid = False
        elif self._size_limit.get().strip() == "custom" and self._frame_size_limit.size_limit.get() == "":
            error_message = "File size limit cannot be empty."
            valid = False
        elif output_path == "":
        ...
        if valid:
            ...
            set_textbox_value(self._textbox_message, error_message)
